...
## Matrix #002 beads-only_cleaned {.tabset}
Refined bead-only matrix

Cleaned beads-only matrix (except viakrome80 8-viability and R720-IgA). If present in the original bead file and I was unable to make them go away, 'Ghost events'--spectral ribbon outliers that dont track with primary fluor-- were cleaned from the control files in a separate experiment by gating on scatter and time and imported back into the main unmixing experiment.
...
Note: In first version of this matrix, I noticed separate, distinct, AF-driven populations appearing positive in the UV channels in unstained cells. I went back and added an AF parameter (unstained_2) by gating on monocytes and assigning U7 as the primary detector. Unstained_cells, the main negative control, was assigned B3 and gated on small lymphocytes, and was insufficient at unmixing all the AF. Unstained 1 and unstained 2 have similar but different signatures. The false positives went away after that. 

## Matrix #004 - Hybrid

First iteration of a hybrid matrix starting with beads only (002), adding AF from #003, and building from there. 
