...
I created a FlowJo layout that allows me to see all 1 x Ns for a given unmixed single stain file. I made a new layout for each matrix iteration - spread, potential errors, and improvements from previous iteration were flagged.

red = bad green = good yellow = better but not great
...
Note: In first version of this matrix, I noticed separate, distinct, AF-driven populations appearing positive in the UV channels in unmixed unstained cells. I went back and added an AF parameter (unstained_cells_2) by gating on monocytes and assigning U7 as the primary detector. Unstained_cells, the main negative control, was assigned B3 and gated on small lymphocytes, and was insufficient to unmix the AF I was seeing in the 1 x Ns. Unstained_cells_1 and unstained_cells_2 have similar but different signatures. The false positives went away after that.

note:

BV711 - CD117 - rare CD303- CDF34 - rare CD10 - rare SB436 - Cd11c - rare BUV661 - CD34 - rarish
...
Current status: I am going through 1 x Ns one by and one and tracking whether the unmixing looks the same or better after replacing with cells.

v1: all beads (except viakrome and R720) and monocyte AF

v2: BUV496 - improved after replacing with cells

v3: BV421 - improved after replacing with cells

v4: BV510 - improved after replacing with cells (brighter on cells)

v5: BUV661 - CD34, errors in APC-CD64 and AF647-IgD channels, try w/cells - cells didnt make those errors go away.

[Unmixed FCS files are HERE](https://app.box.com/folder/336525177002)

BV605-lambda - note: in unmixed singles when gating on monocytes, a BV605+ population nucleates around 10e4. In lymphocytes, lambda expression is continuous but is brightest \< 10e5. monocyte AF isnt being unmixed out of BV605, does is matter? try putting on cells instead of beads. lambda improves on lymphocytes with cell control in some channels, looks better in cytexpert scaling then flowjo

BV750-CD14 - has spread but only in channels assigned with B cell markers so its fine

BV785-CD19 - overunmixing from CD27, slight warpy error into negative in BV421 channel, replacing with cells - fixed with cells

RB670-CD79b - reaching up into RB705-CD24, spread in RB705-IgD, even spread in APCF810-CD15, double positive pop in BB515-CD5 even when gating on lymphs. Trying cells. gating on the brightest cells improved unmixing. there is still a BB515-CD5 double positive population rising from \~10e5, i'll address BB515 next.

BB515-CD5 - put this on cells (brighter on cells), still a CD5+/CD79b+ in the CD79b unmixed single stain.

APV-Vio770-kappa - has some weird trespassing extra bright events, trying on cells

### Matrix #004 - Bcellonly miniMatrix

same 004_v5 matrix but non-Bcellonly fluors omitted

[Unmixed FCS Files are HERE](https://app.box.com/folder/336531063895)

# Notes/To Do/Questions

meeting notes 8/21 34x45 as dump for leukocyte progenitors CD15 is grans, low is monos trans are CD5 low CD10 confirms transitinal get rid of APC-CD64? add CD185 (CXCR5)?

-   we probably need a separate matrix for CLL

-   Could we use the healthy matrix with CLL AF parameters to unmix CLL? try.

-   incorporate wash step for light-chain staining to avoid k/l double positives

TO DO:

-   inventory antibody amounts, see what is running low, check blocks

<!-- -->

-   lookup titrations for CXCR5

-   look at single colors, kappa vs IgA, all red laser fluors against eachoother, include CD20, R720 should peak in 700ish off the red

-   double check BV510 and BUV496

-   make R720 plots

-   Make CLL matrix

-   Mini Experiment: Does IgG, IgA, kappa, lambda staining order matter?

-   check on the instrument, make sure it's still passing QC

-   make Bcellonly mini-matrix for iteration_3, upload, organize the box - done

-   make a mini-matrix for NKBDCM for iteration_4

-   organize titration files and plots
