...
### single-color control .fcs files
...
## Spectral unmixing evaluation

-   Raw single-color control .fcs files were loaded into spectral unmixing experiments to generate spectral unmixing matrices, matrices are numbered 001-00X. 

Matrices were applied to single stained cell files for each fluor in the panel. For every single stain, I generated 1 × N plots, where the primary fluor for a given single stain was plotted against all other unmixed parameters. I initially compared bead-only vs. cell-only matrices to determine which controls, cell or bead, provided optimal unmixing with minimal error. From there, hybrid matrices were constructed and evaluated on 1 x Ns of single stains.

I created a FlowJo layout that allows me to see all 1 x Ns for a given unmixed single-color file.
I made a new layout for each matrix and spread, potential errors, and improvements were flagged.
...
## Unmixing Matrix #001 - Beads-only
First attempt.
...
### CLL Markers on B cells unmixed with 001-beads-only
...
## Unmixing Matrix #002 beads-only, cleaned {.tabset}

Description and files

Cleaned all-bead matrix (except viakrome80 8-viability and R720-IgA). Each bead control was searched for 'ghost events'--spectral ribbon outliers that dont track with primary fluor-- were cleaned from the control files in a separate experiment file by gating on scatter and time and imported back into the unmixing experiment.

[Link to the cloned unmixing experiment with spectral unmixing details PDF](https://app.box.com/folder/335882732501)

## Unmixing Matrix #003 - Cells only with an added AF parameter

003 is entirely cell controls. When I unmixed unstained cells with the first version of this matrix, I noticed separate, distinct, AF-driven populations appearing positive in the UV channels. I went back and added an AF parameter (unstained_2) by gating on monocytes and assigning U7 as the primary detector. Unstained cells, the main negative control, was assigned B3 and gated on small lymphocytes, and was insufficient at unmixing monocyte derived AF. Unstained 1 and unstained 2 have similar but different signatures. The false positives went away after that. 

## Unmixing Matrix #004vx - Hybrid with additional AF

First iteration of a hybrid matrix starting with beads only (002) with added AF parameters from #003, and building from there. 
If a fluor was unmixed better with cells in 003, I replaced the bead file with the cell file. 
I am going through one by and one and tracking whether the unmixing looks the same or better after replacing with cells. 
